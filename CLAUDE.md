...
- `Docs/AmbienteTest.md` — cómo levantar en el server un segundo stack completo (app + Postgres + Redis + una instancia **vacía** de OpenProject) aislado por nombre de proyecto Compose, para probar antes de tocar producción. Archivos: `docker-compose.test.yml` + `.env.test.example`.
