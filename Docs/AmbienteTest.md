...
Lo único que separa test de producción es el `-p trackingtasksop-test`: cambia contenedores,
red y **volúmenes**. Si se olvida el `-p`, Compose reusa el proyecto `trackingtasksop` y pisa
producción con la configuración de test.
...
chmod 600 .env.test
nano .env.test    # OP_SECRET_KEY_BASE y GROQ_API_KEY
```

`OP_SECRET_KEY_BASE`: `openssl rand -hex 32`.
`GROQ_API_KEY`: la misma de producción, o vacía si no se va a probar el bot (el arranque la
exige, pero acepta cualquier cadena).
...
```bash
docker compose -p trackingtasksop-test logs -f openproject
```
...
docker compose -p trackingtasksop-test stop

# Empezar de cero: borra la base de la app Y la de OpenProject
docker compose -p trackingtasksop-test -f docker-compose.yml -f docker-compose.test.yml \
  --env-file .env.test down -v
```
...
El cron que hace `docker compose stop` antes del `shutdown` solo conoce el proyecto de
